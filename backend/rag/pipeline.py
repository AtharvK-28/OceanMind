...
class OceanMindRAG:
    ...
    def _initialise_locked(self):
        ...
        # Two OpenAI-compatible providers are supported. Groq is preferred when
        # its key is present; OpenRouter remains the fallback provider.
        groq_key = os.getenv("GROQ_API_KEY", "")
        openrouter_key = os.getenv("OPENROUTER_API_KEY", "")
        if not groq_key and not openrouter_key:
            logger.warning("No GROQ_API_KEY or OPENROUTER_API_KEY set — RAG will use fallback mode.")
        ...
        # LLM: Groq first — same OpenAI-compatible interface, and the key we
        # actually ship. Falls through to OpenRouter, then to retrieval-only.
