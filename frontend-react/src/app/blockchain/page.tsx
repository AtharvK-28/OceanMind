...
export default function BlockchainPage() {
  ...
  const { data: chain } = useSWR<ChainSummaryResponse>("/api/v1/trace/chain-summary", fetcher);
  ...
  async function handleLog(e: React.FormEvent) {
    ...
    try {
      ...
      setCatchResult(res);
      // Invalidate by key so the activity feed shows the new block at once,
      // rather than waiting out its polling interval.
      await refreshLedger();
